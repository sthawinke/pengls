...
This vignette demonstrates the use of the _gpls_ package for high-dimensional data with spatial or temporal autocorrelation. It consists of an iterative loop around the _nlme_ \parencite{Pinheiro2021} and _glmnet_ \parencite{Friedman2010} packages.
...
The fitting command is similar, this time the $\lambda$ parameter is found through cross-validation of the naive glmnet. We choose $\alpha=0.5$ this time, fitting an elastic net model.

```{r timeFit}
gplsFitTime = gpls(data = dfTime, outVar = "a", verbose = TRUE,
...
The _gpls_ package currently does not execute cross-validation. If the tuning parameter $\lambda$ is not supplied, the optimal $\lambda$ according to cross-validation with the naive _glmnet_ function is used. We encourage the user to supply his own $lambda$ values, preferably choosing it through cross validation.
